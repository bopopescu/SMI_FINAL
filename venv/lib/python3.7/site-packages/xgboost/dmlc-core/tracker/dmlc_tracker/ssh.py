...
One need to make sure all subordinates machines are ssh-able.
"""
...
def sync_dir(local_dir, subordinate_node, subordinate_dir):
    """
    sync the working directory from root node into subordinate node
    """
    remote = subordinate_node[0] + ':' + subordinate_dir
    logging.info('rsync %s -> %s', local_dir, remote)
    prog = 'rsync -az --rsh="ssh -o StrictHostKeyChecking=no -p %s" %s %s' % (
        subordinate_node[1], local_dir, remote)
