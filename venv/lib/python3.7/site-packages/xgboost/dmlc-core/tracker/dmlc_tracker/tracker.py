...
class SubordinateEntry(object):
    def __init__(self, sock, s_addr):
        subordinate = ExSocket(sock)
        self.sock = subordinate
        self.host = get_some_ip(s_addr[0])
        magic = subordinate.recvint()
        assert magic == kMagic, 'invalid magic number=%d from %s' % (magic, self.host)
        subordinate.sendint(kMagic)
        self.rank = subordinate.recvint()
        self.world_size = subordinate.recvint()
        self.jobid = subordinate.recvstr()
        self.cmd = subordinate.recvstr()
        self.wait_accept = 0
        self.port = None
...
class RabitTracker(object):
    ...
    tracker for rabit
    """
    def __init__(self, hostIP, nsubordinate, port=9091, port_end=9999):
        ...
        self.end_time = None
        self.nsubordinate = nsubordinate
    ...
    @staticmethod
    def get_neighbor(rank, nsubordinate):
        rank = rank + 1
        ret = []
        if rank > 1:
            ret.append(rank // 2 - 1)
        if rank * 2 - 1 < nsubordinate:
            ret.append(rank * 2 - 1)
        if rank * 2 < nsubordinate:
            ret.append(rank * 2)
        return ret

    def subordinate_envs(self):
        """
        get enviroment variables for subordinates
        can be passed in as args or envs
        """
        return {'DMLC_TRACKER_URI': self.hostIP,
                'DMLC_TRACKER_PORT': self.port}

    def get_tree(self, nsubordinate):
        tree_map = {}
        parent_map = {}
        for r in range(nsubordinate):
            tree_map[r] = self.get_neighbor(r, nsubordinate)
    ...
    def get_ring(self, tree_map, parent_map):
        ...
        ring_map = {}
        nsubordinate = len(tree_map)
        for r in range(nsubordinate):
            rprev = (r + nsubordinate - 1) % nsubordinate
            rnext = (r + 1) % nsubordinate
            ring_map[rlst[r]] = (rlst[rprev], rlst[rnext])
        return ring_map

    def get_link_map(self, nsubordinate):
        """
        get the link map, this is a bit hacky, call for better algorithm
        to place similar nodes together
        """
        tree_map, parent_map = self.get_tree(nsubordinate)
        ring_map = self.get_ring(tree_map, parent_map)
        rmap = {0 : 0}
        k = 0
        for i in range(nsubordinate - 1):
            k = ring_map[k][1]
            rmap[k] = i + 1
    ...
    def accept_subordinates(self, nsubordinate):
        ...
        while len(shutdown) != nsubordinate:
            fd, s_addr = self.sock.accept()
            s = SubordinateEntry(fd, s_addr)
            if s.cmd == 'print':
                msg = s.sock.recvstr()
            ...
            # lazily initialize the subordinates
            if tree_map is None:
                assert s.cmd == 'start'
                if s.world_size > 0:
                    nsubordinate = s.world_size
                tree_map, parent_map, ring_map = self.get_link_map(nsubordinate)
                # set of nodes that is pending for getting up
                todo_nodes = list(range(nsubordinate))
            else:
                assert s.world_size == -1 or s.world_size == nsubordinate
            if s.cmd == 'recover':
                assert s.rank >= 0
                ...
                if len(pending) == len(todo_nodes):
                    ...
                    for s in pending:
                        ...
                        logging.debug('Recieve %s signal from %s; assign rank %d',
                                      s.cmd, s.host, s.rank)
                if len(todo_nodes) == 0:
                    logging.info('@tracker All of %d nodes getting started', nsubordinate)
    ...
    def start(self, nsubordinate):
        def run():
            self.accept_subordinates(nsubordinate)
...
def submit(nworker, nserver, fun_submit, hostIP='auto', pscmd=None):
    ...
    if nserver == 0:
        rabit = RabitTracker(hostIP=hostIP, nsubordinate=nworker)
        envs.update(rabit.subordinate_envs())
        rabit.start(nworker)
        if rabit.alive():
           fun_submit(nworker, nserver, envs)
    else:
        pserver = PSTracker(hostIP=hostIP, cmd=pscmd, envs=envs)
        envs.update(pserver.subordinate_envs())
        if pserver.alive():
            fun_submit(nworker, nserver, envs)
...
def start_rabit_tracker(args):
    ...
    rabit = RabitTracker(hostIP=get_host_ip(args.host_ip), nsubordinate=args.num_workers)
    envs.update(rabit.subordinate_envs())
